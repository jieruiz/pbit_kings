...
module tb;
    ...
    function automatic logic clamp_pattern(input int unsigned row, input int unsigned col);
        case ((row * 3) + col)
            0, 2, 4, 6, 8: clamp_pattern = 1'b1;
            default:       clamp_pattern = 1'b0;
        endcase
    endfunction
    ...
    task automatic configure_one_node(input int unsigned row, input int unsigned col);
        ...
        write_reg(A_NODE_TARGET, pack_node_target(TARGET_MODE_LOCAL,
                                                  ...
                                                  NODE_TARGET_COL_WIDTH'(seed_col)), "node seed target");
    ...
    task automatic configure_one_edge(
        ...
        write_reg(A_EDGE_CFG, pack_edge_cfg(7'h10, 1'b1, 1'b1), "edge cfg");
    ...
    task automatic configure_3x3_edges();
        for (int r = 0; r < 3; r++) begin
            for (int c = 0; c < 2; c++) begin
                configure_one_edge(EDGE_TYPE_EDGE_H, r, c);
            end
        end

        for (int r = 0; r < 2; r++) begin
            for (int c = 0; c < 3; c++) begin
                configure_one_edge(EDGE_TYPE_EDGE_V, r, c);
            end
        end

        for (int r = 0; r < 2; r++) begin
            for (int c = 0; c < 2; c++) begin
                configure_one_edge(EDGE_TYPE_EDGE_DSE, r, c);
            end
        end

        for (int r = 0; r < 2; r++) begin
            for (int c = 1; c < 3; c++) begin
                configure_one_edge(EDGE_TYPE_EDGE_DSW, r, c);
            end
        end
    endtask
    ...
    task automatic dump_run_debug(input logic [31:0] status);
        $display("[RUN_3X3_DEBUG] status=0x%08h time=%0t", status, $time);
        $display("[RUN_3X3_DEBUG] phase_state=%0d sweep_cnt=%0d num_sweeps=%0d run_busy=%0b run_done=%0b",
        ...
        $display("[RUN_3X3_DEBUG] phase_start={%0b,%0b,%0b,%0b} all_done={%0b,%0b,%0b,%0b}",
        ...
        $display("[RUN_3X3_DEBUG] done_cnt=%0d done_q=%0b cnt_max=%0d num_majority=%0d",
    ...
    task automatic start_run_and_wait_done();
        ...
        do begin
            ...
            if (poll_count > 200) begin
                ...
                $fatal(1, "[RUN_3X3] timeout waiting RUN_DONE, status=0x%08h time=%0t", status, $time);
            end
        end while (!status[RUN_DONE_MSB:RUN_DONE_LSB]);

        if (status[ERROR_MSB:ERROR_LSB]) begin
            error_count++;
            $error("[RUN_3X3] GLOBAL_STATUS.ERROR set, status=0x%08h time=%0t", status, $time);
        end
    endtask
    ...
    task automatic latch_snapshot0();
        ...
        if (!saw_snapshot_latch) begin
            error_count++;
            $error("[RUN_3X3] snapshot latch pulse was not observed time=%0t", $time);
        end
        if (!saw_snapshot_valid) begin
            error_count++;
            $error("[RUN_3X3] snapshot valid pulse was not observed time=%0t", $time);
        end
    endtask
    ...
    task automatic check_3x3_snapshot();
        ...
        for (int r = 0; r < 3; r++) begin
            for (int c = 0; c < 3; c++) begin
                ...
                if (actual !== expected) begin
                    error_count++;
                    $error("[RUN_3X3] spin[%0d][%0d]=%0b expected=%0b time=%0t", r, c, actual, expected, $time);
                end
            end
        end
    endtask
    ...
    initial begin
        ...
        if (error_count == 0) begin
            $display("[TB_RUN_3X3] PASS");
        end else begin
            $fatal(1, "[TB_RUN_3X3] FAIL error_count=%0d", error_count);
        end
